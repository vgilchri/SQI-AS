...
ideal_to_isogeny_power_of_two2 := function(I, J, K, phi_J, phi_K,isom_K,epsilon:other_side_isogeny:=[],other_side_ideal:=lideal<O0|1>);
...
gen_keys2 :=function()
	...
	repeat
		...
		try
			phi_K,isom_K,J,phi_J:=ideal_to_isogeny_power_of_two2(K,lideal<O0|1>,lideal<O0|1>,[],[],[],0);
		catch e
			foundsol:=false;
		end try;
	until foundsol;
...
monty_subtract:=function(P,Q);
	...
	x3:= x3 / y3;
	z3:= 1 / y3;
	return SemiMontgomeryXZ(x3,z3,SemiMontgomery(M));
end function;
